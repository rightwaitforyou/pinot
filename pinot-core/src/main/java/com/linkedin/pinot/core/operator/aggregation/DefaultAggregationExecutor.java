...
public class DefaultAggregationExecutor implements AggregationExecutor {
   ...
   */
  @Override
  public void aggregate(int[] docIdSet, int length) {
    Preconditions
  ...
  private void aggregateColumn(AggregationFunctionContext aggrFuncContext, AggregationResultHolder resultHolder,
    ...
    switch (aggrFuncName) {
      ...
      case AggregationFunctionFactory.DISTINCTCOUNTHLL_AGGREGATION_FUNCTION:
        aggregationFunction.aggregate(length, resultHolder, _dataFetcher.getHashCodeArrayForColumn(aggrColumn));
        break;

      default:
        aggregationFunction.aggregate(length, resultHolder, _dataFetcher.getValueArrayForColumn(aggrColumn));
        break;
    }
  }
